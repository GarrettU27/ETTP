...
def convert_name(arrhythmia_id: int):
    if arrhythmia_id == 1:
...
def cull_database(arrhythmia_ids: List[int]):
    ...
    for temp_id in arrhythmia_ids:
        ...
        while num_selected < 200:
            ...
            WHERE p1.id = d1.patient_id AND
                d1.arrhythmia_id = """ + str(temp_id) + """ AND
                (
                    SELECT COUNT(*)
                    FROM patient p2, diagnosis d2
                    WHERE p2.id = d2.patient_id AND
                    p2.id = p1.id AND
                    d2.arrhythmia_id <> """ + str(temp_id) + """
                ) = """ + str(additional_diagnosis)
                                    ).fetchall()

            for ecg_data in query_res:
                try:
                    _ = plot_12_ecgs(ecg_data[1], convert_name(temp_id))
                    # Insert into non-delete table
                    cur.execute("INSERT INTO goodecgs VALUES(NULL, ?, ?)", (ecg_data[0], temp_id))
                    con.commit()
